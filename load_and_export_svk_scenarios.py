...
if create_price_scenarios:
    ...
    create_df = lambda n: eur_to_sek*load_svk_scenarios(n, columns, area=area, wy=weather_year).rename('ElPrice-SEK')
    ...
    current_prices = pd.read_csv(folder / "electricity_prices.csv", parse_dates=['Timestamp'], index_col=['Timestamp']).loc[year_to_use,:]
    # look for the first monday
    weekdays = current_prices.index.weekday.values
    mondays = np.where(weekdays==0)[0]
    first_monday = mondays[0]
    last_idx = first_monday+8736
    current_prices = current_prices.iloc[first_monday:last_idx,]
    # export the corresponding dataframe
    new_index = get_hourly_time_index(2019)
    current_prices.index = new_index
    if to_save:
        current_prices.to_csv('Data/CreatedScenarios/DAprice-scenarios/price_{}.csv'.format(2019), index_label='Timestamp')
